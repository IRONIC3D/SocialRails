...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should create a status for the current user when logged in" do
    ...
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:carole).id }
    end

    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:iyad).id
  end
  ...
  test "should update status when current user is logged in" do
    ...
    put :update, id: @status, status: { content: @status.content, user_id: users(:carole).id }
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:iyad).id
  end
